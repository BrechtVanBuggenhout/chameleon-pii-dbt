-- Content scan must catch the email planted in the free-text notes column.
...
select 'notes EMAIL not detected by content scan' as failure
from check_
where not exists (
  select 1 from {{ ref('pii_content_findings') }}
  where table_name = 'pii_ci_customers' and column_name = 'notes' and pattern = 'EMAIL'
)
