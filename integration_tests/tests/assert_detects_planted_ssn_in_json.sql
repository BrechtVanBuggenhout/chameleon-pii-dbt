-- Content scan must catch an SSN buried inside a nested key of a JSON
...
select 'nested SSN in JSON payload not detected by content scan' as failure
from check_
where not exists (
  select 1 from {{ ref('pii_content_findings') }}
  where table_name = 'stg_ci_json_probe' and column_name = 'payload'
    and source_data_type = 'JSON' and pattern = 'SSN'
)
